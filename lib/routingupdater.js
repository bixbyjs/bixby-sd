...
RoutingUpdater.prototype.register = function(hostname, rrtype, rr, cb) {
  ...
  (function niter(i) {
    ...
    (function riter(j) {
      ...
      resolver.module.register(name, rrtype, rr, function(err, records) {
...
RoutingUpdater.prototype.deregister = function(hostname, rrtype, rr, cb) {
  var services = this._services
    , names = [ hostname ]
    , parts = hostname.split('.');
  if (parts.length == 1) {
  ...
  (function niter(i) {
    ...
    (function riter(j) {
      ...
      resolver.module.deregister(name, rrtype, rr, function(err, records) {
        if (err) { return riter(j - 1); }
        return cb(null, records);
      });
    })(services.length - 1);
