 ...
 * @class
 */
function PortResolver(registry) {
 ...
 */
PortResolver.prototype.resolve = function(hostname, rrtype, cb) {
 ...
 * });
 */
PortResolver.prototype.resolveSrv = function(hostname, cb) {
  ...
  if (!entry || !entry.port) {
    error = new Error('querySrv ENODATA ' + hostname);
    error.code = 'ENODATA';
    return cb(error);
  }
